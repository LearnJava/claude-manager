...
как `task_source` — цикл сам остановится, когда файл опустеет.

Правила для сессии:

1. Если в таблице Status есть задача `● IN PROGRESS` — продолжай ЕЁ (она была прервана).
2. Иначе возьми ВЕРХНИЙ указатель из `LEARN-STATUS.md`, все зависимости которого
   уже `✓ DONE`. Отметь её в таблице Status: `○ TODO` → `● IN PROGRESS`.
3. Выполни задачу по её спецификации в этом файле. Каждая функция — с тестом.
4. Завершение (только при зелёных `go build ./...`, `go test ./...` и, если
   менялся фронтенд, `npm --prefix frontend test`):
   - удали строку-указатель задачи из `LEARN-STATUS.md`;
   - в таблице Status: `● IN PROGRESS` → `✓ DONE (дата)`;
   - обнови документацию по матрице doc-sync из `CLAUDE.md` (новый биндинг →
     строка в таблице биндингов; новое поле конфига → `config.example.toml`);
   - закоммить.
...
[[session]]
name = "LN"
prompt = "Прочитай CLAUDE.md и LEARN-TASKS.md. Следуй разделу «Как сессия берёт задачу»: продолжи ● IN PROGRESS-задачу или возьми верхний доступный указатель из LEARN-STATUS.md. Одна задача = одна сессия."
...
| LN-05 | ○ TODO | internal/experience/primer.go, internal/session/session.go, config/types.go |
| LN-06 | ○ TODO | internal/experience/journal.go, internal/analysis/journal.go |
...
| LN-17 | ○ TODO | internal/experience/mdlog.go, indexer.go (IngestDir), testdata/logfiles/ |
