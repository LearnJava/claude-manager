...
### HR-04a. Конец хода ≠ конец задачи — ✓ сделано (для обоих runtime)
Найдено на живой S6 (Lumen): Hermes оборвал PERF-14 по своему `agent.max_turns: 150`, отдал
`exit_code 0`, и менеджер засчитал задачу (`tasks_done++`, статус `completed`). В логах проекта
то же самое у Claude: «API Error», «жду фоновую сборку» — всё засчитывалось как выполненное.

- **Задача закрыта** ⇔ её указатель ушёл из очереди `task_source`. Очередь читается из
  `origin/<main>` после `git fetch` (туда её вливает сессия из своего слота), без remote — с диска.
  Верхний указатель запоминается до хода и сравнивается после (`task_outcome.go`).
- Указатель на месте + маркер `continue_session` → `slice` (срез многосрезовой задачи, успешный ход).
  Указатель на месте, маркера нет → `unfinished`: без `tasks_done`, без `task_done`-уведомления,
  строка `system` в логе с причиной. 3 `unfinished` подряд → сессия встаёт в `error`, а не
  перезапускает ту же задачу бесконечно.
- Без `task_source` — поведение прежнее: каждый чистый выход = выполненная задача.
- Hermes: `--max-turns` из `hermes_max_turns` (0 → 500 вместо 150 из конфига Hermes). Обрыв по
  лимиту виден только в `state.db` (служебное сообщение `MAX_ITERATIONS_SUMMARY_REQUEST`; уведомления
  о фоновых процессах после него пропускаются — так было на реальном обрыве). Читается только на
  чтение, после каждого автономного хода; в лог — строка «Turn stopped by Hermes' step limit».
- Claude: `result.subtype` ≠ `success` (например `error_max_turns`) теперь пишется в строку
  `result` лога: `[error_max_turns] …`.
- Статусы `slice`/`unfinished` в `session_runs`: деньги и токены идут в дневную статистику,
  задача — только за `completed`; журнал пишется и за `slice`.

### HR-04b. Лимит, спрятанный за 401, и мягкий стоп — ✓ сделано
Найдено на живой S6: пятичасовой лимит кончился (429 на рабочем токене), Hermes переключился на
второй токен из своего пула (`claude_code`, давно отозванный) и отдал ход с
`HTTP 401: OAuth access token has been revoked`. Менеджер увидел обычную ошибку; мягкий стоп,
нажатый двумя часами раньше, сработал на этой ошибке, и сессия встала насовсем.

- Неудачный ход Hermes проверяется по `<HERMES_HOME>/logs/agent.log` (хвост 4 МБ): строки этого
  разговора после начала хода с `RateLimitError` / `Credential 429` / `rate_limit_error` → ход
  считается лимитом (`EvtRateLimit`, статус `rate_limited`, ожидание `rate_limit_pause` и новая
  попытка), в лог строка «Hermes hit the API usage limit (429)».
- После лимита или ошибки следующая попытка продолжает тот же разговор (`--resume`), а не
  начинает задачу заново.
- Мягкий стоп срабатывает только на границе задачи: до первого запуска, после закрытой задачи или
  законченного среза. Ошибка, лимит или `unfinished` его не исполняют — сессия ждёт и продолжает.
  Для claude-сессий так же.
- Тест `TestHermesRuntime_RateLimitBehind401_ResumesDespiteSoftStop` повторяет S6 на fakehermes
  (`FAKEHERMES_FAIL_429_THEN_401=1`) и падает со старым поведением мягкого стопа.
