...
#
# Зачем: менеджер умеет запускать сессию с флагом `--worktree`, но он делает
# БЕЗЫМЯННЫЙ worktree от HEAD на каждый запуск процесса. Одна прерванная задача
# (rate limit, 403, перезапуск приложения) — и следующая сессия стартует в
# чистом дереве от master, не видит уже сделанной работы и делает её заново.
# 2026-09-08 так было потрачено ~$12: задача LN-01 реализована семью сессиями
# в семи одноразовых worktree, ни одна не доехала до master.
#
# Слот решает это тем, что состояние задачи живёт в git, а не в памяти
# менеджера: ветка `p<N>-<task>` в слоте переживает и обрыв сессии, и рестарт
# приложения. Следующая сессия видит её через `git branch` и продолжает.
#
# Второй эффект — `frontend/node_modules` (и любой другой негитовый кэш)
# остаётся на месте: `git worktree remove` сносит его вместе с каталогом, и
# следующая задача платит `npm install` заново.
#
# Использование:
...
#
#   slot   — каталог в .claude/worktrees/: p1-work, p2-work, ...
#   branch — ветка задачи (p<N>-<task>); существующая — переключаемся на неё,
#            новая — создаётся от base
#   base   — база для новой ветки, по умолчанию master
#
...
# из чеклиста завершения выполняется ПОСЛЕ release. Сам слот при этом остаётся.
# На `master` слот встать не может: master занят главным деревом — только
# detached.
#
...
#   2. свои коммиты, не влитые в master → отказ (сначала влей или удали ветку);
...
if [ "${1:-}" = "list" ]; then
  printf '%-12s %-32s %-7s %s\n' SLOT BRANCH DIRTY NODE_MODULES
  found=0
  for d in "$POOL"/*-work; do
    ...
    m=cold
    [ -d "$d/frontend/node_modules" ] && m=warm
    printf '%-12s %-32s %-7s %s\n' "$(basename "$d")" "$b" "$n" "$m"
  done
...
'git worktree prune' и удали каталог вручную."
  ...
  echo "Слот $slot создан и занят веткой $branch."
  echo 'frontend/node_modules пуст — если задача трогает фронтенд, нужен npm --prefix frontend install.'
  printf '%s\n' "$path"
  exit 0
fi
...
# --- Гард 2: свои коммиты, не влитые в master ---
...
[ -d "$path/frontend/node_modules" ] && echo 'frontend/node_modules на месте.' \
                                    || echo 'frontend/node_modules пуст — фронтовые тесты потребуют npm install.'
printf '%s\n' "$path"
