<script lang="ts">
    ...
    async function loadConfig() {
        ...
        try {
            ...
            error = $t('mixedRun.loadFailed', { error: e?.message ?? String(e) });
        } finally {
            loading = false;
        }
    }
    ...
    async function runDispatch() {
        ...
        if (!selectedProject || !selectedWorker) {
            error = $t('mixedRun.dispatch.selectProjectAndWorker');
            return;
        }
        const id = briefID.trim() || `brief-${Date.now()}`;
        if (!briefTask.trim()) {
            error = $t('mixedRun.dispatch.enterTask');
            return;
        }
        try {
            const result = await dispatchTask(selectedProject, id, briefTask.trim(), selectedWorker);
            info = $t('mixedRun.dispatch.finished', { id: result.ID, status: result.Status });
        } catch (e: any) {
            error = $t('mixedRun.dispatch.failed', { error: e?.message ?? String(e) });
        }
    }
    ...
    async function onCancel(taskID: string) {
        try {
            ...
            error = $t('mixedRun.tasks.cancelFailed', { error: e?.message ?? String(e) });
        }
    }
    ...
    function roundVerdict(r: RoundRecord): { label: string; cls: string } {
        if (r.ParseError) return { label: $t('mixedRun.tasks.verdictParseError'), cls: 'text-status-error' };
        if ((r.Rejected?.length ?? 0) > 0) return { label: $t('mixedRun.tasks.verdictRejected'), cls: 'text-status-error' };
        if (r.Passed) return { label: $t('mixedRun.tasks.verdictPassed'), cls: 'text-status-working' };
        return { label: $t('mixedRun.tasks.verdictGatesFailed'), cls: 'text-status-ratelimit' };
    }
...
<div
    ...
    <div
        ...
        <div class="px-4 py-3 border-b border-bg-border flex items-center justify-between shrink-0">
            <h2 class="text-text font-semibold text-base">{$t('mixedRun.title')}</h2>
            <button
        ...
        <div class="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-5">
            {#if loading}
                <div class="text-text-muted text-sm italic py-10 text-center">{$t('common.loading')}</div>
            {:else if mixedProjects.length === 0}
                <div class="text-text-muted text-sm py-10 text-center">
                    {$t('mixedRun.noProjects')}
                    <b>mixed_programming</b>
                    {$t('mixedRun.noProjectsSuffix')}
                </div>
            {:else}
                <!-- ───────── Dispatch form ───────── -->
                <section class="bg-bg-elevated border border-bg-border rounded p-3 space-y-3">
                    <h3 class="text-text font-semibold text-sm">{$t('mixedRun.dispatch.heading')}</h3>
                    <div class="grid grid-cols-2 gap-3">
                        <label class="flex flex-col text-xs text-text-muted gap-1">
                            {$t('mixedRun.dispatch.project')}
                            <select
                        ...
                        <label class="flex flex-col text-xs text-text-muted gap-1">
                            {$t('mixedRun.dispatch.worker')}
                            <select
                                data-testid="mixed-worker"
                                bind:value={selectedWorker}
                                class="bg-bg border border-bg-border rounded px-2 py-1 text-sm text-text">
                                {#if workers.length === 0}
                                    <option value="">{$t('mixedRun.dispatch.noWorkers')}</option>
                                {/if}
                                {#each workers as w}
                    ...
                    <label class="flex flex-col text-xs text-text-muted gap-1">
                        {$t('mixedRun.dispatch.briefId')}
                        <input
                            type="text"
                            bind:value={briefID}
                            placeholder={$t('mixedRun.dispatch.briefIdPlaceholder')}
                            class="bg-bg border border-bg-border rounded px-2 py-1 text-sm text-text font-mono" />
                    </label>
                    <label class="flex flex-col text-xs text-text-muted gap-1">
                        {$t('mixedRun.dispatch.brief')}
                        <textarea
                            data-testid="mixed-brief"
                            bind:value={briefTask}
                            rows="3"
                            placeholder={$t('mixedRun.dispatch.briefPlaceholder')}
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            ...
                            {$dispatching ? $t('mixedRun.dispatch.running') : $t('mixedRun.dispatch.button')}
                        </button>
                        <p class="text-xs text-text-dim">
                            {$t('mixedRun.dispatch.hint')}
                        </p>
                    </div>
                </section>
                ...
                {#if $workerActivity.length > 0}
                    <section>
                        <div class="flex items-center justify-between mb-1">
                            <h3 class="text-text font-semibold text-sm">{$t('mixedRun.activity.heading')}</h3>
                            <button
                                type="button"
                                on:click={clearActivity}
                                class="text-xs text-text-muted hover:text-text">{$t('mixedRun.activity.clear')}</button>
                        </div>
                        <div
                ...
                <!-- ───────── Quality report ───────── -->
                <section>
                    <h3 class="text-text font-semibold text-sm mb-2">{$t('mixedRun.quality.heading')}</h3>
                    {#if $mixedQuality.length === 0}
                        <div class="text-xs text-text-muted italic">{$t('mixedRun.quality.empty')}</div>
                    {:else}
                        <table class="w-full text-xs" data-testid="mixed-quality">
                            <thead class="text-text-muted">
                                <tr class="text-left border-b border-bg-border">
                                    <th class="py-1 pr-2">{$t('mixedRun.quality.colWorker')}</th>
                                    <th class="py-1 px-2">{$t('mixedRun.quality.colDoneTotal')}</th>
                                    <th class="py-1 px-2">{$t('mixedRun.quality.colNeedsHuman')}</th>
                                    <th class="py-1 px-2" title={$t('mixedRun.quality.colAvgRoundsTitle')}>{$t('mixedRun.quality.colAvgRounds')}</th>
                                    <th class="py-1 px-2" title={$t('mixedRun.quality.colCleanPatchesTitle')}>{$t('mixedRun.quality.colCleanPatches')}</th>
                                    <th class="py-1 px-2">{$t('mixedRun.quality.colParseErrs')}</th>
                                    <th class="py-1 pl-2">{$t('mixedRun.quality.colGateFails')}</th>
                                </tr>
                            </thead>
                ...
                <!-- ───────── Task timelines ───────── -->
                <section>
                    <h3 class="text-text font-semibold text-sm mb-2">{$t('mixedRun.tasks.heading')}</h3>
                    {#if sortedTasks.length === 0}
                        <div class="text-xs text-text-muted italic">{$t('mixedRun.tasks.empty')}</div>
                    {:else}
                            ...
                            {#each sortedTasks as task (task.ID)}
                                <li class="bg-bg-elevated border border-bg-border rounded p-3">
                                    <div class="flex items-center justify-between mb-2">
                                        <div class="text-sm">
                                            <span class="font-mono text-text">{task.BriefID}</span>
                                            <span class="text-text-muted"> · {task.WorkerName}</span>
                                            <span class={`ml-2 font-medium ${statusColor(task.Status)}`}>{task.Status}</span>
                                        </div>
                                    ...
                                    {#if task.Branch}
                                        <div class="text-xs text-text-dim font-mono mb-2">{$t('mixedRun.tasks.branch', { branch: task.Branch })}</div>
                                    {/if}
                                    {#if task.Error}
                                        <div class="text-xs text-status-error mb-2">{$t('mixedRun.tasks.error', { error: task.Error })}</div>
                                    {/if}

                                    <ol class="space-y-1">
                                        {#each task.Rounds ?? [] as r (r.Number)}
                                            {@const v = roundVerdict(r)}
                                            <li class="text-xs">
                                                <div class="flex items-center gap-2">
                                                    <span class="text-text-muted">{$t('mixedRun.tasks.round', { number: r.Number })}</span>
                                                    <span class="text-text-dim">
                                                        {$t('mixedRun.tasks.appliedRejected', { applied: (r.Applied?.length ?? 0), rejected: (r.Rejected?.length ?? 0) })}
                                                    </span>
                                                    <span class={`font-medium ${v.cls}`}>{v.label}</span>
                                                    {#if r.Gates?.Commands}
                                                        <button
                                                            type="button"
                                                            on:click={() => toggleGate(task.ID, r.Number)}
                                                            class="text-text-muted hover:text-text underline">
                                                            {expandedGates[`${task.ID}#${r.Number}`] ? $t('mixedRun.tasks.gatesHide') : $t('mixedRun.tasks.gatesShow')}
                                                        </button>
                                                    {/if}
                                                </div>
                                                {#if r.ParseError}
                                                ...
                                                {/if}
                                                {#if expandedGates[`${task.ID}#${r.Number}`]}
        ...
        <div class="px-4 py-3 border-t border-bg-border flex items-center justify-between shrink-0">
            <div class="text-xs">
                ...
                {:else}
                    <span class="text-text-muted">{$t('mixedRun.footer.worktreesNote')}</span>
                {/if}
            </div>
            <button
                type="button"
